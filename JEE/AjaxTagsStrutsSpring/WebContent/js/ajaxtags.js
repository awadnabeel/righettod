...
ResponseXmlToHtmlLinkListParser.prototype = Object.extend(new AbstractResponseParser(), {
  ...
  parse: function() {
    var ul = '<ul class="' + this.treeClass + '">';
    var root = this.xml.documentElement;
    ...
    if (responseNodes.length > 0) {
      ...
      for (var i=0; i<itemNodes.length; i++) {
        ...
        if (nameNodes.length > 0 && valueNodes.length > 0) {
          ...
          ul += '<li><img id="img_' + value + '" class="' + this.collapsedClass + '">';
          ul += '<a href="' + url + '" class="' + this.nodeClass + '">' + name + '</a>';
          ul += '<div id="' + value + '"></div></li>';
          if(!collapsed)
...
AjaxJspTag.Tree.prototype = Object.extend(new AjaxJspTag.Base(), {
  ...
  execute: function(e) {
    ...
    var aj = new Ajax.Request(this.url, {
      ...
      parameters: params,
      onSuccess: function(request) {
         obj.options.parser.load(Object.extend(request, {collapsedClass: obj.options.collapsedClass,
  ...
  handler: function(request, options) {
	...
	$(options.target).innerHTML = parser.content;   
	var images = $(options.target).getElementsByTagName("img");
	for (var i=0; i<images.length; i++) {
	  //get id
	  var id = images[i].id.substring(4);
